    ...
    for w in load_closed_class_from_quran():
        ...
        if normalize_for_root_extraction is not None:
            try:
                ...
                pass
    # Extra closed-class forms (data/contracts/lists/extra_closed_class.csv)
    # — ظُروف، حُروف نِداء مُرَكَّبَة، إِنَّ مَع ضَمائر، ضَمائر مُنفَصِلَة.
    try:
        from contracts_loader import _load_rows
        rows = _load_rows("extra_closed_class.csv", subdir="lists")
        for r in rows:
            form = (r.get("form") or "").strip()
            if not form:
                continue
            out.add(_strip_diac(form))
            if normalize_for_root_extraction is not None:
                try:
                    out.add(_strip_diac(normalize_for_root_extraction(form)))
                except Exception:
                    pass
    except Exception:
        pass
