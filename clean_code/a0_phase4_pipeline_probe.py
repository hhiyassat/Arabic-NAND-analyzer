...
# PROJECT_ROOT defaults to the clean_code/ dir holding this script (so the
# probe is runnable without setting ARABIC_ANALYZER_ROOT). i3rab_engine,
# relation_extractor, event_extractor all live directly under that root.
PROJECT_ROOT = Path(os.environ.get(
    "ARABIC_ANALYZER_ROOT",
...
EXPECTED_PROJECT_FILES: list[str] = [
    "i3rab_engine/engine.py",
    "i3rab_engine/layer1.py",
    "relation_extractor.py",
    "event_extractor.py",
    "arabic_analyzer/contextual_resolver/integration.py",
]

# Ayah subset for the real sweep. Small enough to finish in seconds, large
# enough to exercise hundreds of Layer-1 calls and produce non-zero phase4
# eligibility hits. Override via env var if you need a bigger run.
A0_SUBSET_AYAHS = int(os.environ.get("A0_SUBSET_AYAHS", "120"))
QURAN_DATA_PATH = Path(os.environ.get(
    "ARABIC_ANALYZER_QURAN_PATH",
    str(PROJECT_ROOT.parent / "data" / "quran-uthmani-with-pause-mark.txt"),
)).resolve()
...
def _load_subset_verses() -> list[tuple[int, int, str]]:
    """Read A0_SUBSET_AYAHS rows from the pipe-separated Quran text file."""
    verses: list[tuple[int, int, str]] = []
    if not QURAN_DATA_PATH.exists():
        raise FileNotFoundError(
            f"Quran data file not found at {QURAN_DATA_PATH}. "
            "Set ARABIC_ANALYZER_QURAN_PATH to override."
        )
    with open(QURAN_DATA_PATH, encoding="utf-8") as fh:
        for line in fh:
            ...
            try:
                ...
                continue
            if len(verses) >= A0_SUBSET_AYAHS:
                break
    return verses


def run_pipeline() -> dict[str, Any]:
    """In-process A0 sweep mirroring analyze_verse_v3.main.

    Runs I3rabEngine → RelationExtractor → EventExtractor for the first
    A0_SUBSET_AYAHS verses of the pipe-separated Quran file. The tracer
    counts Layer-1 calls; this serializer emits the minimal per-ayah
    structure extract_metrics() expects (tokens / events / relations).

    Field-name choices match the real class definitions:
      RelationGraph.relations  (NOT .edges)
      Relation.name            (e.g. "agent_of"  — relation name)
      Relation.kind_type       (e.g. "subj_pred" — relation family)
      Relation.kind            (proof certainty Certificate/Hypothesis/Zero)
      EventGraph.events
      Event.verb_surface       (verb surface form — Event has NO .head)
      Event.type               (verb lemma)
      Event.verb_position, Event.tense
      TokenI3rab.phase4        (dict of phase4_* keys, empty by default)
    """
    ...
    verses = _load_subset_verses()
    print(f"[A0] running {len(verses)} ayahs (subset)")
    ...
    for surah, ayah_num, text in verses:
        try:
            sent = engine.analyze_sentence(text)
        except Exception as exc:  # noqa: BLE001
            print(f"[A0] engine skip {surah}:{ayah_num}: {exc}", file=sys.stderr)
            continue

        try:
            rg = rel_ext.extract(sent)
        except Exception as exc:  # noqa: BLE001
            print(f"[A0] rel skip {surah}:{ayah_num}: {exc}", file=sys.stderr)
            rg = None

        try:
            eg = ev_ext.extract(sent, rg) if rg is not None else None
        except Exception as exc:  # noqa: BLE001
            print(f"[A0] ev skip {surah}:{ayah_num}: {exc}", file=sys.stderr)
            eg = None

        tokens_list: list[dict[str, Any]] = []
        for t in (getattr(sent, "tokens", None) or []):
            phase4 = getattr(t, "phase4", None)
            if not isinstance(phase4, dict):
                phase4 = {}
            tokens_list.append({
                "surface": getattr(t, "token", "") or "",
                "word_class": getattr(t, "word_class", "") or "",
                "wordclass_kind": getattr(t, "wordclass_kind", "") or "",
                "phase4": phase4,
            })

        relations_list: list[dict[str, Any]] = []
        if rg is not None:
            for r in (getattr(rg, "relations", None) or []):
                relations_list.append({
                    "name": getattr(r, "name", "") or "",
                    "kind_type": getattr(r, "kind_type", "") or "",
                    "proof_kind": getattr(r, "kind", "") or "",
                })

        events_list: list[dict[str, Any]] = []
        if eg is not None:
            for ev in (getattr(eg, "events", None) or []):
                events_list.append({
                    "verb_surface": getattr(ev, "verb_surface", "") or "",
                    "verb_lemma": getattr(ev, "type", "") or "",
                    "verb_position": getattr(ev, "verb_position", None),
                    "tense": getattr(ev, "tense", "") or "",
                })
...
def extract_metrics(payload: Any) -> dict[str, int]:
    ...
    for ayah, tok in _iter_tokens(payload):
        ...
        # Keys are the prefixed names produced by
        # arabic_analyzer.contextual_resolver.integration.maybe_apply_resolver
        # and propagated through i3rab_engine.engine._extract_phase4_metadata.
        if phase4.get("phase4_selected_function"):
            m["phase4_selected_function_count"] += 1
        if phase4.get("phase4_certainty") == "Certificate":
            m["phase4_certificate_count"] += 1
        if phase4.get("phase4_certainty") == "Hypothesis":
            m["phase4_hypothesis_count"] += 1
        if phase4.get("phase4_source") == "ContextualAmbiguityResolver":
            m["contextual_resolver_applications"] += 1
        if phase4.get("phase4_unresolved_ambiguous"):
            m["unresolved_ambiguous_count"] += 1
        # qa_zero / interrogative_marker / segmentation_leak: not yet on
        # TokenI3rab. Buckets stay at 0 until upstream layers expose them.
        if tok.get("qa_zero"):
            m["qa_zeros"] += 1
    ...
    # Event serializer emits verb_surface (Event has no .head). Keep both
    # paths so older payload shapes still register if encountered.
    for _ayah, ev in _iter_events(payload):
        m["event_count"] += 1
        head = (ev.get("head") or {}).get("surface") if isinstance(ev.get("head"), dict) else ""
        verb_surface = ev.get("verb_surface") or head or ev.get("surface") or ""
        if "ئك" in verb_surface:
            m["event_ik_count"] += 1
        if ev.get("is_fragment"):
            m["fragment_event_count"] += 1

    # Relation buckets (unresolved/relative/conditional) are not modelled on
    # relation_schema.Relation today (Relation.kind is proof-certainty,
    # Relation.name is the relation name like "agent_of"). The counts stay
    # at 0 until those categories are introduced — that is honest, not a bug.
    for _ayah, rel in _iter_relations(payload):
        m["relation_count"] += 1
        bucket = rel.get("name") or rel.get("kind_type") or rel.get("kind") or rel.get("type")
        if bucket == "unresolved":
            m["unresolved_edges"] += 1
        elif bucket == "relative":
            m["relative_edges"] += 1
