...
def _match_pattern(wazn: str, surface: str, pattern: str) -> bool:
    ...
    if pattern == "tafdeel_afal":
        # اسم تَفضيل عَلى وَزن أَفْعَل — تَمييز ضَيِّق لِتَجَنُّب الخَلط مَع
        # أَفعال المُضارِع المُتَكَلِّم (أَعلَمُ، أَكتُبُ).
        # الشُّروط (يَجِب تَوَفُّر إِحدى الإِشارات الصَّريحَة):
        #   1) تَنوين (ٌ ٍ ً) — صَريح كَ اسم مُعرَب
        #   2) أَلِف مَقصورَة (ى) أَو خَنجَريَّة (ٰ) — يَنتَهي بِها كَثير
        #      مِن أَسماء التَّفضيل المَعتَلَّة (أَدنَى، أَعلى، أَقسى)
        if not surface:
            return False
        check = surface
        for clitic in ("وَ", "فَ", "أَوَ", "أَفَ"):
            if check.startswith(clitic) and len(check) > len(clitic):
                check = check[len(clitic):]
                break
        if not check.startswith("أَ"):
            return False
        if len(check) < 5:
            return False
        clean = check.rstrip("ٓ")
        if not clean:
            return False
        # شَرط صارِم: تَنوين أَو أَلِف مَقصورَة/خَنجَريَّة
        has_tanwin = any(c in clean for c in {"ٌ", "ٍ", "ً"})
        ends_in_alif_maqsura = clean[-1] in {"ى", "ٰ"}
        if not (has_tanwin or ends_in_alif_maqsura):
            return False
        return wazn.startswith("أَ")
...
class VerbalWaznContract:
    ...
    def classify(self, wazn: str, *, surface: str = "") -> dict:
        ...
        # اسم قَطعًا. الأَفعال لا تَقبَل التَّنوين أَبَدًا. هذا يَمنَع
        # تَصنيف «تِجَٰرَةً» وَ «حَاضِرَةً» وَ نَحوها كَ FIIL.
        if surface and any(c in surface for c in ("ٌ", "ٍ", "ً")):
            return {
                "kind": "nominal",
                "aspect": "",
                "rule_id": "HARD_GATE_TANWIN",
                "note": "التَّنوين عَلى السَّطح — الأَفعال لا تَقبَل التَّنوين",
            }
