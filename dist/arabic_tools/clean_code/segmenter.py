...
def _rule_future_particle(st: _State) -> _State:
    ...
    Refuses to fire after IV/DET have been stripped (those signal the س is
    part of the stem, not a future particle — e.g. يَسْتَطِيعُ has س as root letter).
    """
    ...
    i = _consume_diacritics(chars, i)
    rest = "".join(chars[i:])
    # Future particle should be followed by an imperfect verb (which starts
    # with ي/ت/ن/أ). If the next letter is not one of these, refuse the strip.
    if _count_letters(rest) < _MIN_STEM_LETTERS:
        return st
