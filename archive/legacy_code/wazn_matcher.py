...
def _load_upstream_normalizer():
    ...
    Registers the module in sys.modules before exec to satisfy dataclass
    ...
    here = Path(__file__).resolve().parent
    candidates = [
        here / "normalizer.py",                                   # sibling (canonical)
        here.parent / "clean_code" / "normalizer.py",
        here.parents[1] / "clean_code" / "normalizer.py",
...
# =========================================================================
# Upstream segmenter (sibling to normalizer). The segmenter provides a
# wazn-aware-friendly stem hypothesis. We use it as ONE additional variant
# (high priority) without removing existing variant safety nets.
# =========================================================================
def _load_upstream_segmenter():
    """Locate segmenter.py and return its segment() function (or None)."""
    import importlib.util
    import sys as _sys
    here = Path(__file__).resolve().parent
    candidates = [
        here / "segmenter.py",
        here.parent / "clean_code" / "segmenter.py",
        here.parents[1] / "clean_code" / "segmenter.py",
        Path("/Users/husseinhiyassat/fractal/hussein/clean_code/segmenter.py"),
        Path("/sessions/nice-epic-cannon/mnt/hussein/clean_code/segmenter.py"),
    ]
    for p in candidates:
        if p.is_file():
            mod_name = "_upstream_segmenter_module"
            spec = importlib.util.spec_from_file_location(mod_name, p)
            if spec is None or spec.loader is None:
                continue
            mod = importlib.util.module_from_spec(spec)
            ...
            try:
                spec.loader.exec_module(mod)
                return getattr(mod, "segment", None)
            except Exception:
                _sys.modules.pop(mod_name, None)
                continue
    return None
...
_upstream_segment = _load_upstream_segmenter()


def _maybe_segment_stem(text: str):
    """Return the segmenter's stem hypothesis (or None if unavailable)."""
    if _upstream_segment is None or not text:
        return None
    try:
        # Let the segmenter do its own normalization (canonical diacritic order,
        # alif-madda decomposition, etc.) — the input to generate_variants may
        # be only NFC-normalized, missing later steps.
        result = _upstream_segment(text, normalize_input=True)
        stem = result.stem
...
def generate_variants(word: str) -> list[tuple[str, str, float]]:
    ...
    # Tier 0.5: upstream segmenter's stem hypothesis (cost 0.03)
    # The segmenter applies full morphological clitic-stripping (article,
    # conjunctions, prepositions, IV/PV/pronoun suffixes, hamzat wasl, etc.).
    # When it produces a different stem from the original, that stem is a
    # high-confidence variant — try it before any of the brittle tier-1+
    # rule-based variants below.
    seg_stem = _maybe_segment_stem(word)
    if seg_stem and seg_stem != word:
        add("segmenter_stem", seg_stem, 0.03)
        # Also a no-final-iraab variant of the segmenter stem
        seg_no_irab = _strip_final_diacritic(seg_stem)
        if seg_no_irab != seg_stem:
            add("segmenter_stem_no_irab", seg_no_irab, 0.04)
